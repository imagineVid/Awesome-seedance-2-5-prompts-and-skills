/**
 * [INPUT]: 依赖 markdown-generator 的英文发布接口与 Seedance 2.5 数据契约
 * [OUTPUT]: 验证英文首屏、事实边界、产品链接、可点击视频案例、编辑前后帧和分类行为
...
const categories: FilterCategory[] = [
  ...
  {
    id: 72,
    title: "Long-Take Blocking & Camera Paths",
    slug: "long-take-blocking-camera-paths",
    parentId: 4,
...
test("publishes all reviewed Seedance 2.5 language surfaces", () => {
...
test("renders a Seedance 2.5 empty-state README without inherited model copy", () => {
  const markdown = generateMarkdown(
    { all: [], featured: [], regular: [], stats: { total: 0, featured: 0 }, categories },
    0
  );

  assert.match(markdown, /Awesome Seedance 2.5 Prompts & Skills/);
  assert.match(markdown, /public Seed model catalog does not yet provide a complete Seedance 2\.5 model card/);
  assert.match(markdown, /seed\.bytedance\.com\/en\/models/);
  assert.match(markdown, /imaginevid\.io\/seedance-2-0/);
  assert.doesNotMatch(markdown, /Nano Banana|Gemini 2\.5 Flash Image|Raycast/i);
});

test("uses the verified ImagineVid product route", () => {
  assert.equal(getSeedanceProductUrl(), "https://imaginevid.io/seedance-2-0");
  assert.equal(getSeedanceProductUrl("ja"), "https://imaginevid.io/seedance-2-0");
});

test("groups a video prompt by its single production workflow", () => {
  const groups = groupPromptsByWorkflow([prompt()], categories);
  assert.equal(groups.length, 1);
  assert.equal(groups[0].category.slug, "long-take-blocking-camera-paths");
...
test("shows source and result frames for a video editing case", () => {
  ...
  assert.match(markdown, /Reference and result frames/);
...
test("keeps the English source prompt below a reviewed localization", () => {
  ...
  assert.match(markdown, /Prompt original de la fuente/);
