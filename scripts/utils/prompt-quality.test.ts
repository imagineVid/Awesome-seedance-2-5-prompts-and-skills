/**
 * [INPUT]: 依赖 prompt-quality 的 Seedance 2.5 评分、硬门槛与去重接口
...
test("accepts a fully attributed Seedance 2.5 video record", () => {
  const result = scoreStoredTwitterPrompt({
    ...
    sourceMeta: {
      ...
      model_evidence: "seedance-2.5",
    },
    videoCategories: { workflows: [{ slug: "dialogue-foley-music-timing" }] },
  });

  assert.equal(result.decision, "accept");
});

test("accepts a detailed Seedance 2.5 candidate with video evidence", () => {
...
test("rejects a launch announcement without a reusable shot brief", () => {
  const result = scoreTwitterCandidate({
    id: "announcement",
    text: "Seedance 2.5 is now available. Try it today!",
