/**
 * [INPUT]: 依赖 twitterapi.io 候选字段、本地视频提示词指纹与 Seedance 2.5 模型证据
...
export function scoreStoredTwitterPrompt(prompt: StoredTwitterPrompt): QualityResult {
  ...
  if (
    prompt.sourceMeta?.model_evidence !== "seedance-2.5"
  ) {
    hardFailures.push("missing verified Seedance 2.5 model evidence");
  }
...
export function scoreTwitterCandidate(candidate: TwitterCandidate): QualityResult {
  ...
  const hasSeedanceEvidence = /\bseedance[-\s]*2[.\s-]*5\b/i.test(
    relevanceText
  );
  const namesDifferentVideoModel = /\b(?:veo|kling|seedance[-\s]*(?:1|2\.0|2\.1)|sora|hailuo|runway|luma|grok[-\s]*imagine)\b/i.test(
    relevanceText
  ) && !hasSeedanceEvidence;
  if (hasPromptSignal) {
    score += 30;
  ...
  if (!hasSeedanceEvidence) {
    reasons.push("missing explicit Seedance 2.5 evidence; manual review required");
