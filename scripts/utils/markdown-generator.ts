/**
 * [INPUT]: 依赖 prompt-repository 的视频提示词契约、多语言 i18n 真源与 workflow-copy 分类说明
 * [OUTPUT]: 对外提供十四种 README、可点击视频案例、媒体表格、状态说明、分类分组和 CTA 的 Markdown 生成能力
 * [POS]: scripts/utils 的核心展示层，把 Seedance 2.5 结构化数据渲染为 GitHub 原生页面
...
export const SUPPORTED_LANGUAGES: LanguageConfig[] = [
  ...
  { code: "zh", name: "简体中文", readmeFileName: "README.zh.md" },
...
const REPO = "imagineVid/Awesome-seedance-2-5-prompts-and-skills";
const REPO_URL = `https://github.com/${REPO}`;
const SEEDANCE_PRODUCT_URL = "https://imaginevid.io/seedance-2-0";
const BYTEDANCE_MODEL_CATALOG_URL = "https://seed.bytedance.com/en/models";
const RELEASE_REPORT_URL = "https://www.theinformation.com/briefings/bytedance-unveils-seedance-2-5-video-model";

export function getSeedanceProductUrl(locale = "en"): string {
  void locale;
  return SEEDANCE_PRODUCT_URL;
}
...
function generateHeader(locale: string): string {
  const coverImage = "public/images/seedance-2-5-prompt-skills-cover.png";

  return `<a href="${REPO_URL}">
  <img src="${coverImage}" alt="Seedance 2.5 Prompt Skills" width="100%" />
</a>
...
**[${t("browseGallery", locale)}](${getSeedanceProductUrl(locale)})**
...
function generatePromptSection(
  ...
  md += `**[${getPromptCtaLabel(locale)}](${getSeedanceProductUrl(locale)})**\n\n`;
  md += "---\n\n";
...
${t("learnMore", locale)} [Seedance 2.5 release reporting](${RELEASE_REPORT_URL}) · [ByteDance Seed model catalog](${BYTEDANCE_MODEL_CATALOG_URL}) · [Available Seedance workflow on ImagineVid](${getSeedanceProductUrl(locale)})
...
**[${t("viewInGallery", locale)}](${getSeedanceProductUrl(locale)})** •
